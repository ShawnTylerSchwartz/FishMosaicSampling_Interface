<?php 
	...
	// Adjacent to gill slit of fish (1)
	...
	$file_output_counter++;
	// Tailend of fish along SL line (shifted inwards 10% of fish SL) (4)
	...
	execute_crop_fish($_GET['image'], $scaled_width, $scaled_height, $topmost_pos_x, ($topmost_pos_y+$height_edge_shift_factor), $mosaic_box_w, 5, $user_email);
	$file_output_counter++;
	...
	echo '<script type="text/javascript">alert("' . $file_output_counter . ' Fish mosaic samples has been successfully saved! Resetting interface...");</script>';
	...
	*/
	function execute_crop_fish($fish, $fish_ws, $fish_hs, $pos_x, $pos_y, $fish_mosaic_w, $boxnum, $useraddress) {
		...
		if(!file_exists($image_file)) {
   			...
   			$fh = fopen($txt, 'a'); 
    		$txt=$file.','. $exportPath . $name . ',' . $seshID . ',' . round($fish_ws) . 'x' . round($fish_hs) . ',' . $readableDate . ',' . $seshID . ',' . $useraddress . '<hr />'; 
